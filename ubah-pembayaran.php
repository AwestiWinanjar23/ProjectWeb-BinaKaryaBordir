              ...
              <div class="col-lg-12 mb-4">
                ...
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-3 font-weight-bold text-primary">Form Ubah Data Pembayaran:</h6>
                        <div class="card-body">
                            <form method="POST" action="ubah-pembayaran-aksi.php">
                                <?php
                                include "koneksi.php";      
                                $ID_Pembayaran=$_GET['ID_Pembayaran'];                        
                                $query=mysqli_query($connect, "SELECT * FROM tb_pembayaran where ID_Pembayaran='$ID_Pembayaran'");
                                while($data = mysqli_fetch_array($query)) {
                                    ?>
                                    <div class="mb-3">
                                        <select name="ID_Pemesanan" class="form-select form-control">
                                            <?php
                                            $q=mysqli_query($connect, "SELECT * from tb_pemesanan");
                                            while ($d = mysqli_fetch_array($q)) { ?>
                                            <option value="<?=$d['ID_Pemesanan'];?>"<?=$d['ID_Pemesanan'] == $data['ID_Pemesanan'] ? "selected" : ""?>><?php echo $d['ID_Pemesanan'];?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleFormControlInput1" class="form-label">Ongkir</label>
                                        <input name="Ongkir" type="number"  class="form-control" id="exampleFormControlInput1" placeholder="Rp." name="Ongkir" value=<?php echo $data['Ongkir']; ?>>
                                        <input name="ID_Pembayaran" type="hidden" value=<?php echo $data['ID_Pembayaran']; ?> class="form-control" id="exampleFormControlInput1" placeholder="Rp." name="Ongkir">
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleFormControlInput1" class="form-label">Total*</label>
                                        <input name="Total" type="number" class="form-control" id="exampleFormControlInput1" placeholder="Rp." name="Total" value=<?php echo $data['Total']; ?>>
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleFormControlInput1" class="form-label">Status Pembayaran</label><br>
                                        <div class="form-check">
                                            <input name="Status_Pembayaran" class="form-check-input" type="radio" name="flexRadioDefault" id="flexRadioDefault1" value=<?php echo $data['Status_Pembayaran']; ?><?=$data['Status_Pembayaran'] == "Bayar" ? " checked" : ""?>>
                                            <label class="form-check-label" for="flexRadioDefault1">Bayar</label>
                                        </div>
                                        <div class="form-check">
                                            <input name="Status_Pembayaran" value="Belum" class="form-check-input" type="radio" name="flexRadioDefault" id="flexRadioDefault2" value=<?php echo $data['Status_Pembayaran']; ?><?=$data['Status_Pembayaran'] == "Belum" ? " checked" : ""?>>
                                            <label class="form-check-label" for="flexRadioDefault2">Belum</label>
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        <label for="formFile" class="form-label">Bukti Pembayaran</label>
                                        <input name="Bukti_Pembayaran"class="form-control" type="file" id="formFile" value=<?php echo $data['Bukti_Pembayaran']; ?>></div>
                                    </div>
                                    <?php } ?>
